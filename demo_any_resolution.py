...
parser.add_argument('--input_dir', default='../unet-pytorch-origin/img/test/DIC/', type=str, help='Input images')
...
parser.add_argument('--result_dir', default='./img/test/result/dapi/', type=str, help='Directory for results')
parser.add_argument('--weights',
                    default='./model/model-dapi.pth', type=str,
...
model = SUNet_model(img_size=256,
                    patch_size=4,
                    in_chans=3,
                    out_chans=3,
                    embed_dim=96,
...
for file_ in files:
    ...
    img = cv2.imread(file_, cv2.COLOR_BGR2RGB)
    input_ = TF.to_tensor(img).unsqueeze(0).cpu()
    with torch.no_grad():
        # pad to multiple of 256
        square_input_, mask, max_wh = overlapped_square(input_.cpu(), kernel=model_img, stride=stride)
    ...
    save_img((os.path.join(out_dir, f + '.tif')), restored)
    print(f"{f}.tif has been saved")
